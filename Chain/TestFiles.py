...
def random_tile():
    """
    Create a string of the format T12ABC with 12 and ABC being chosen randomly.
    :return: String compliant with reg T[0-9]{2}[A-Z]{3}
    """
    import string
    letters = string.ascii_uppercase
    return "T" + str(random.randint(0, 99)).zfill(2) + ''.join(random.choice(letters) for _ in range(3))


def random_platform(product_level=None):
    if not product_level:
        product_level = random.choice(["L1C", "L2A", "L3A"])
...
class DummyGenerator(object):
    ...
    """
    def __init__(self, root, date=random_date(), tile=random_tile(), platform=random_platform()):
        self.root = root
        self.date = date
        self.tile = tile
...
class DummyEarthExplorer(DummyGenerator):
    ...
    def __init__(self, root, date=random_date(), tile=random_tile(), platform=random_platform()):
        ...
        if platform[:2].upper() == ["S2"] or platform[:9].upper() == "SENTINEL2":
            self.platform = "S2"
        elif platform[:2].upper() == ["L8"] or platform[:8].upper() == "LANDSAT8":
            self.platform = "L8"
        elif platform[:2].upper() == ["VE"] or platform[:8].upper() == "VENUS":
            self.platform = "VE"
        else:
